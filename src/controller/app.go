...
func NewAppControllerImpl(logger *zap.SugaredLogger, appService app.AppService, actionService action.ActionService, attrg *ac.AttributeGroup, treeStateService state.TreeStateService) *AppControllerImpl {
...
func (impl AppControllerImpl) CreateApp(c *gin.Context) {
	// fetch needed param
	...
	app := model.NewAppWithCreateAppRequest(teamID, userID, req)

	// create app
	errInCreateApp := impl.Storage.AppStorage.Create(app)
	if errInCreateApp != nil {
		FeedbackInternalServerError(c, ERROR_FLAG_CAN_NOT_CREATE_APP, "create app error: "+errInCreateApp.Error())
		return
	}

	// init tree_states
	treeState := model.NewTreeStateByApp(teamID, userID, app)
	errInCreateTreeState := impl.Storage.TreeStateStorage.Create(treeState)
	if errInCreateTreeState != nil {
		FeedbackInternalServerError(c, ERROR_FLAG_CAN_NOT_CREATE_APP, "can not create tree state: "+errInCreateTreeState.Error())
		return
	}

	// init scheme
	if req.IsRequestWithScheme() {
		initScheme := req.ExportInitScheme()
		for _, v := range initScheme {
			componentNode := NewComponentNodeByRawData(v)
			impl.TreeStateStorage()
...
func (impl AppControllerImpl) DeleteApp(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
...
func (impl AppControllerImpl) RenameApp(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	appID, errInGetAPPID := GetMagicIntParamFromRequest(c, PARAM_APP_ID)
	userID, errInGetUserID := GetUserIDFromAuth(c)
...
func (impl AppControllerImpl) ConfigApp(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	appID, errInGetAPPID := GetMagicIntParamFromRequest(c, PARAM_APP_ID)
	userID, errInGetUserID := GetUserIDFromAuth(c)
...
func (impl AppControllerImpl) GetAllApps(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	userAuthToken, errInGetAuthToken := GetUserAuthTokenFromHeader(c)
...
func (impl AppControllerImpl) GetMegaData(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	appID, errInGetAPPID := GetMagicIntParamFromRequest(c, PARAM_APP_ID)
...
func (impl AppControllerImpl) DuplicateApp(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	appID, errInGetAPPID := GetMagicIntParamFromRequest(c, PARAM_APP_ID)
	userID, errInGetUserID := GetUserIDFromAuth(c)
...
func (impl AppControllerImpl) ReleaseApp(c *gin.Context) {
	// fetch needed param
	teamID, errInGetTeamID := GetMagicIntParamFromRequest(c, PARAM_TEAM_ID)
	appID, errInGetAPPID := GetMagicIntParamFromRequest(c, PARAM_APP_ID)
	userAuthToken, errInGetAuthToken := GetUserAuthTokenFromHeader(c)
